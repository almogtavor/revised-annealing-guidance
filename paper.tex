...
\newcommand{\almg}{\textcolor{red}{\mathbb{almg}}}
\newcommand{\tal}{\textcolor{blue}{\mathbb{tal}}}
\newcommand{\orz}{\textcolor{green}{\mathbb{orz}}}
...
We build directly on \citet{dahary2025annealing}, who introduce a tiny scheduler $w_\theta(t,\|\delta_t\|,\lambda)$ trained against an EMA-balanced $\delta$/$\epsilon$ loss to anneal the CFG++ guidance scale per step.
\citet{fsg2025} (NeurIPS 2025) reframe guidance as fixed-point iteration and show that the early denoising stages dominate quality, motivating their Foresight Guidance (FSG) operator that performs a small number of repeated long-interval calibrations at a few early sites; we adopt their FSG inference operator on top of the annealing scheduler.
\citet{learn2guide2025} introduce a self-consistency MMD objective on the guided iterates, which we integrate on the \emph{final} FSG iterate so the MLP learns to be calibrated under FSG; following both works, we also extend the MLP input with a normalized interval $(t-s)/T$ and a projected pooled prompt embedding $c_{\text{emb}}$.

\section{Adapting the Method to Flow-Matching Transformers}

Porting the proposed method from a DDPM-based UNet (SDXL) to a flow-matching transformer (Stable Diffusion 3 Medium) required several non-trivial modifications. While the high-level structure of the algorithm remains unchanged, the underlying parameterization, sampling dynamics, and memory constraints differ substantially. Below, we describe the key necessary adaptations.

\subsection{CFG++ Renormalization under Flow Matching}

In DDPM/DDIM, the clean-image estimate is recovered from the predicted noise using the variance schedule. In the original DDIM formulation used in \citep{dahary2025annealing}, the estimate is obtained by subtracting the predicted noise component from the noisy latent:
\begin{equation}
z_0^{\text{DDIM}} =
\frac{z_t - \sqrt{1 - \bar{\alpha}_t}\,\hat{\epsilon}_t}{\sqrt{\bar{\alpha}_t}}.
\end{equation}
The key operation is the subtraction of the noise contribution from $z_t$. Since the forward diffusion process is
\begin{equation}
z_t = \sqrt{\bar{\alpha}_t}\,z_0 + \sqrt{1-\bar{\alpha}_t}\,\epsilon,
\end{equation}
subtracting the noise term $\sqrt{1-\bar{\alpha}_t}\,\epsilon$ removes the stochastic component and leaves only the scaled clean signal, which is then rescaled by $1/\sqrt{\bar{\alpha}_t}$ to recover $z_0$.
...
This preserves the key CFG++ property: the clean estimate is computed using the guided prediction ($\hat{v}_t$), while the stochastic renoising direction is taken from the unconditional model ($\epsilon_t^{\varnothing}$), exactly mirroring the DDIM-based formulation of:
\begin{equation}
...
The epsilon loss (Eq.~\ref{eq:eps_loss} in \citep{dahary2025annealing}) becomes a velocity loss:
\begin{equation}
...
The delta loss (Eq.~\ref{eq:delta_loss} in \citep{dahary2025annealing}) depends only on $\delta_t$ and therefore remains structurally identical:
\begin{equation}
L_t^{\delta} =
...
\end{equation}
However, the magnitudes of $L_t^{\delta}$ and $L_t^{v}$ (the flow-matching analog of $L_t^{\epsilon}$) can differ by orders of magnitude and shift during training, making the fixed $\lambda$ weighting unreliable: when $L_t^{\delta} \ll L_t^{v}$, the $\delta$-loss gradient is effectively drowned out regardless of $\lambda$.
...
\Input{ \BlankLine
  ...
  $\sigma_t = t/T$: noise level at step $t$
}
...
\Repeat{converged}{
  ...
  $\mathcal{L} \gets \lambda\,\|\delta_{t-1}\|^2 + (1 - \lambda)\,\|\hat{v}_t - v_{\text{GT}}\|^2$\;
  Take gradient step on $\nabla_\theta \mathcal{L}$ \tcp*{Update scheduler}
}
\end{algorithm}

\section{Auto-$\lambda$: Geometry-Driven Guidance Selection}
At inference, $\lambda$ is normally chosen by hand to trade quality for prompt adherence. We replace this manual choice with a per-step \emph{geometry-driven} value computed directly from the conditional/unconditional alignment, $\lambda_t = \mathrm{clip}\!\left(\tfrac{1+\cos(v_t^{\varnothing},\delta_t)}{2},\,0,\,1\right)$, where $\delta_t = v_t^c - v_t^{\varnothing}$. Intuitively, when the conditional correction is well aligned with the unconditional flow ($\cos\!\to\!1$) the model can safely amplify it; when they oppose ($\cos\!\to\!-1$) the correction is suppressed; orthogonal corrections produce $\lambda_t=0.5$. Since $\lambda_t$ is fed into the same MLP that was trained on user-chosen $\lambda$, this requires no retraining and adds no model evaluations.

\section{FSG at Inference for Annealing Guidance}
We import the FSG site schedule of \citet{fsg2025} on top of our learned scheduler: a small set $\{(t_i, K_i, \Delta t_i)\}_{i=1}^{3}$ of early calibration sites with iteration counts $K=(3,2,2)$ and a fixed interval $\Delta t = 0.125\,T$. At each site we run $K_i$ inner iterations of (guided forward to $s_i$, unconditional inverse back to $t_i$) using $w_\theta$ to refine $z_{t_i}$, then take the actual scheduler step from the refined latent. All other steps remain plain annealed CFG++. At inference we also pass the normalized interval $(t-s)/T$ and the pooled prompt embedding $c_{\text{emb}}$ to $w_\theta$ when those inputs are present; for legacy checkpoints they default to zero and the call reduces to the original signature.

\begin{algorithm}[t]
\caption{Annealing Inference with FSG (3 early sites, $K=(3,2,2)$)}
\label{alg:fsg_inference}
\KwIn{$T$ sampling steps; trained $w_\theta$; condition $c$; site set $\{(t_i, K_i, s_i=t_i-\Delta t)\}_{i=1}^{3}$ with $\Delta t = 0.125\,T$.}
$z_T \sim \mathcal{N}(0, I)$\;
\For{$i = 1$ \KwTo $T$}{
  \If{$t_i$ \textbf{is the closest sampling step to an FSG site} $(t_*,K_*,s_*)$}{
    $z \gets z_{t_i}$\;
    \For{$k=1$ \KwTo $K_*$}{
      $v^{\varnothing}, v^{c} \gets v(z,t_i,\varnothing), v(z,t_i,c)$;\quad $\hat v \gets v^{\varnothing}+w_\theta(t_i,\delta,\lambda,(t_i-s_*)/T,c_{\text{emb}})\,(v^{c}-v^{\varnothing})$\;
...
To make the scheduler aware of the FSG operator that will be used at inference, we add an \emph{extended} training mode that mixes 50\% standard one-step training with 50\% FSG-aligned training: when the sampled $t$ falls inside the early FSG region $[0,\,0.375\cdot 1000]$, we snap it to the nearest FSG site, run the matching $K_i$-iteration FSG block, and minimize a self-consistency term $\lambda_{\text{SC}}\,\mathrm{MMD}^2(\{z_{t_i}^{(K_i)}\},\{z_{t_i}^{(0)}\})$ on the \emph{final} FSG iterate, where $z_{t_i}^{(0)}$ are directly noised site latents and the minibatch MMD uses the energy kernel $k(x,y)=-\|x-y\|_2$ (following \citet{learn2guide2025}); the rest of the loss is unchanged. The MLP itself is extended with two extra positional-style features, the normalized interval $(t-s)/T$ and a learned linear projection of the pooled prompt embedding $c_{\text{emb}}\in\mathbb{R}^{2048}\!\to\!\mathbb{R}^{4}$, so that the same scheduler can predict $w$ for both regular CFG++ steps and FSG-calibration steps.
Formally, for a fixed FSG site $t_i>s_i$, let $(x_0,c)\sim p_{0,c}$, let $z_{t_i}^{(0)}\sim p_{t_i|0}(\cdot\mid x_0)$ be the directly noised latent at that site, and let $z_{t_i}^{(K_i)}\sim \hat p^{\theta,K_i}_{t_i\mid t_i,c}(\cdot\mid z_{t_i}^{(0)},c)$ denote the final latent after $K_i$ guided forward/inverse FSG refinements driven by $w_\theta$.
The self-consistency condition is that, conditional on the same $(x_0,c)$, the guided site marginal matches the true noising marginal: $\hat p^{\theta,K_i}_{t_i\mid t_i,c}(\cdot\mid z_{t_i}^{(0)},c)\approx p_{t_i|0}(\cdot\mid x_0)$.
We optimize the minibatch estimator of $\mathrm{MMD}^2(P,Q)=\mathbb{E}_{x,x'\sim P}[k(x,x')]+\mathbb{E}_{y,y'\sim Q}[k(y,y')]-2\mathbb{E}_{x\sim P,y\sim Q}[k(x,y)]$ with $k(x,y)=-\|x-y\|_2$, using $P=\{z_{t_i}^{(K_i)}\}$ and $Q=\{z_{t_i}^{(0)}\}$ at the final FSG iterate.

\begin{algorithm}[t]
\caption{Extended-MLP Training with FSG Self-Consistency MMD}
\label{alg:extended_training}
\KwIn{Frozen velocity model $v$; trainable $w_\theta$ with extra inputs $\{(t-s)/T,c_{\text{emb}}\}$; FSG sites $\{(t_i,K_i,\Delta t)\}_{i=1}^{3}$; SC weight $\lambda_{\text{SC}}$.}
\Repeat{converged}{
  Sample $(z_0,c),\,t\sim U[1,1000],\,\lambda\sim U[0,1]$;\quad form $z_t,\,v_{\text{gt}}$\;
  \eIf{$\mathrm{Bernoulli}(0.5)=1$ \textbf{and} $t\le 375$}{
    \tcp{--- FSG-aligned branch ---}
    Snap $t$ to the nearest FSG site $(t_*,K_*,s_*)$; rebuild $z_{t_*}$\;
    Run $K_*$ FSG inner iterations on $z_{t_*}$ with $w_\theta(t_*,\delta,\lambda,(t_*-s_*)/T,c_{\text{emb}})$, obtaining the final iterate $z_{t_*}^{(K_*)}$\;
    Recompute $v^{\varnothing},v^{c}$ at $z_{t_*}^{(K_*)}$ and take one CFG++ step to get $z_{t_*-1}$\;
    $\mathcal{L}\gets\lambda\,\|\delta_{t_*-1}\|^2 + (1-\lambda)\,\bar{r}\,\|\hat v_{t_*}-v_{\text{gt}}\|^2 + \lambda_{\text{SC}}\,\mathrm{MMD}^2(\{z_{t_*}^{(K_*)}\},\{z_{t_*}^{(0)}\})$ \tcp*[r]{energy-kernel SC on final iterate only}
  }{
    \tcp{--- Regular one-step branch (with extra inputs) ---}
    Run one CFG++ step using $w_\theta(t,\delta,\lambda,(t{-}t{-}1)/T,c_{\text{emb}})$\;
    $\mathcal{L}\gets\lambda\,\|\delta_{t-1}\|^2 + (1-\lambda)\,\bar{r}\,\|\hat v_t-v_{\text{gt}}\|^2$\;
  }
