#!/bin/bash
#SBATCH --job-name=sd3-sanity-delta
#SBATCH --nodes=1
...
#SBATCH --output=logs/sanity/slurm_sd3_sanity_delta_%j.log
#SBATCH --error=logs/sanity/slurm_sd3_sanity_delta_%j.log
#SBATCH --partition=killable
#SBATCH --nodelist=n-801,n-802,n-804,n-805,n-806,n-601,n-602

# Sanity check: train with lambda=1 (delta loss only).
# Expected: w should explode to very high values.
...
CONFIG_PATH="scripts/config_sd3_sanity_delta_only.yaml"
if [[ "$USE_CFG" -eq 1 ]]; then
	CONFIG_PATH="scripts/config_sd3_sanity_delta_only_cfg.yaml"
fi

MASTER_PORT=$((29500 + RANDOM % 1000))
echo "=== SANITY CHECK: delta loss only (lambda=1) ==="
